...
public class TransactionService {
    ...
    public BigDecimal getTotalWithdrawn(){
        List<Transaction> transactions = transactionRepository.findAllByType(Type.WITHDRAWN);
        return transactions.stream()
                .map(Transaction::getAmount)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
    }
    ...
    public int getTransactionsCountByUser(String userId, String type){
        UserDTO user = userService.get(userId);
        Type trans = Type.valueOf(type);
        return transactionRepository.countAllByTypeAndUser(trans, mapper.map(user, User.class));
    }
